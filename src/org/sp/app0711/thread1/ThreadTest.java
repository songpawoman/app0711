...
2) 쓰레드의 생명주기(LifeCycle)
   ...
   - Runnable 상태로 진입시킨다 : start() 호출  
     jvm의 실행대상이 된다 즉 실행부호가 된다
   - jvm 내부 알고리즘에 의해 랜덤하게 특정 Thread 가 선택되고, 이 선택된
     쓰레드의 run() 메서드를 실행하는 단계를 Running 이라 한다 
   - 소멸: run() 메서드의 닫는 } 브레이스를 만나면, 쓰레드를 소멸 됨, 이때의
...
public class ThreadTest extends JFrame {
	...
	public ThreadTest() {
		...
		//이벤트 연결 코드는 재사용성이 없기 때문에, 되도록이면 코드가 간소화될수록
		//좋다..따라서 이벤트 구현시 내부익명클래스를 지원함...
		//java8 람다(Lambda) : 굳이 이벤트 연결 코드를 객체로까지 갈필요있는가?
		//                              그냥 함수로 표현하는 방법을 지원함..
		//                              ex) js 에서의 화살표함수랑 흡사..
		//final int x=9;
	...
	//왜????? 메인쓰레드는 1) gui 에서 사용자의 이벤트처리, 그래픽처리
	...
	public static void main(String[] args) {
		...
		//했다는 메시지가 출력된다..결론) 자바의 실행부라 불렷던 그 녀석은 사실 
		//메인 쓰레드였다...메인쓰레드는 프로그램을 운영하는 실행부이므로, 
		//반드시 대기상태나, 루프에 빠트리지 말자!!(프로그램 먹통 됨)
		/*
		int[] arr=new int[2];
		arr[0]=5;
		arr[1]=7;
